...
// Removes a node with a certain data element
template<typename T>
bool LinkedList<T>::remove(T myData) {
    if (indexOf(myData) == -1) {            //check to see if input data exists
        return false;
    } else {
        int index = indexOf(myData);
        if (myData == removeAt(index)) {    //try to remove value at value index
            return true;
        }
        return false;
    }
...
// Returns the index of the last instance of an element
template<typename T>
int LinkedList<T>::lastIndexOf(T myData) const {
    ...
    return last ;                             // return -1 if value not found
}

// Checks if a certain value exists in the linked list
template<typename T>
